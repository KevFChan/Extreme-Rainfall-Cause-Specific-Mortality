...
```{r}
#Now we want to add the holiday data
holid <- read.csv("/Users/kevin/Documents/ChenLabResearch/HurricaneProject/Coding/DataSources/us_holidays.csv")

#Convert the date column into date data types
holid$Date <- as.Date(holid$Date, "%Y-%m-%d")

#And let's clean holid2 to get only the 2015-2018 years
holid <- holid[holid$Date >= "2015-01-01", ]
holid <- holid[holid$Date <= "2018-12-31", ]

#Not really concerned about Holiday.Name
holid <- holid[, -3]

#We can also get rid of the instances of days that aren't holidays
holid <- holid[holid$Holiday.Flag == "True", ]

#And we can pull out the dates
holidates <- holid$Date

#Now let's create a column in mydata for holiday
mydata$holiday <- NA

#Indicate the holidays
mydata$holiday[mydata$date %in% holidates] <- 1

#Now we set the other 
mydata$holiday[is.na(mydata$holiday)] <- 0
```
    ...
    for(j in 0:7){
      ...
      if(death_type == "totdeath"){
        mod1 <- glm(subdata[, death_type] ~ as.factor(subdata[, paste0("prcp95th_lag", j)]) 
                    + ns(time, 4*6) + ns(subdata[, paste0("templag", j)], 5) + dow + holiday,
...
holder$Analysis <- "Holiday"
#write.csv(holder, "/Users/kevin/Documents/ChenLabResearch/HurricaneProject/Coding/PostJournalComments/Holiday.csv")
```
