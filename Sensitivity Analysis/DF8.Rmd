    ...
    for(j in 0:7){
      ...
      if(death_type == "totdeath"){
        mod1 <- glm(subdata[, death_type] ~ as.factor(subdata[, paste0("prcp95th_lag", j)]) 
                    + ns(time, 4*8) + ns(subdata[, paste0("templag", j)], 5) + dow,
...
allmetaresult$Analysis <- "DateDF8"
#holder <- allmetaresult
#write.csv(holder, "/Users/kevin/Documents/ChenLabResearch/HurricaneProject/Coding/PostJournalComments/DateDF8.csv")
```
