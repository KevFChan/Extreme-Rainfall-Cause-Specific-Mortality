---
title: "95th Percentile County-Specific Definition"
...
alldata<-left_join(mdata,lagresult,by=c("FIPS","date"))

#Because we want to get rid of the index column
mydata <- alldata[,-1]

#And since we are redefining the indicator for extreme rainfall, we get rid of the prcp95th lags
mydata <- mydata[, -c(22:30)]
```

```{r}
#And we apply the threshold of exposure such that it is specific to each county
countyFips <- unique(mydata$FIPS)

dailyCT2 <- data.frame()

for(i in countyFips){
  #Subsetting temporary dataframe based on county
  tempData <- mydata[mydata$FIPS == i, ]
  
  #Assigning the thresholds to each county dataframe
  tempData[, "prcp_95th"] <- 0
  tempData[tempData$prcp >= quantile(tempData$prcp, 95/100), "prcp_95th"] <- 1

  dailyCT2 <- rbind(dailyCT2, tempData)
}

mydata <- dailyCT2

#Lag the precipitation definitions within each county
for(j in 0:7){
  mydata[, paste0("prcp95th_lag", j)] <- tsModel::Lag(mydata[, "prcp_95th"], j, group = as.factor(mydata$FIPS))
}
```
  ...
  for(i in seq(length(dlist))){
    ...
    for(j in 0:7){
      ...
      if(death_type == "totdeath"){
        ...
        mod1 <- glm(subdata[, death_type] ~ as.factor(subdata[, paste0("prcp95th_lag", j)]) 
                    + ns(date, 4*6) + ns(subdata[, paste0("templag", j)], 5) + dow,
                    data = subdata, family = quasipoisson)
      }
...
#Get rid of the NAs and redefine the lags and vars
death_results<-na.omit(temp)
vars <- c("totdeath", "death_nonacc", "death_CVD", "death_RSD", "death_external")
lags <-  c("prcp95th_lag0","prcp95th_lag1","prcp95th_lag2","prcp95th_lag3","prcp95th_lag4","prcp95th_lag5","prcp95th_lag6", "prcp95th_lag7")
...
holder$Analysis <- "CountyPercentile"
```

```{r}
#Code to save the results above
#write.csv(holder, "/Users/kevin/Documents/ChenLabResearch/HurricaneProject/Coding/PostJournalComments/CountySpecificDefinitionResults.csv")
```
