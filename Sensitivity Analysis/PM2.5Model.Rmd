...
```{r}
#Load the PM2.5 data from the EPA and getting rid of the first column containing the indices
pm2.5 <- read.csv("/Users/kevin/Documents/ChenLabResearch/HurricaneProject/PM2.5Data.csv")
pm2.5 <- pm2.5[, -1]
pm2.5$Date <- as.Date(pm2.5$Date)
names(pm2.5) <- c("date", "FIPS", "PM25")

#Now merging the data frame with my data using the FIPS first and then the date
mydata <- dplyr::left_join(mydata, pm2.5, by = c("FIPS", "date"))
```

```{r}
#Let's get the counties where we do have PM2.5 measurements
pm25Data <- na.omit(mydata)
pm25County <- unique(pm25Data$FIPS)
```


```{r}
#This block is for not incorporating PM2.5 data
...
regions <- as.character(unique(pm25Data$FIPS))
dlist <- lapply(regions,function(x) pm25Data[pm25Data$FIPS==x,])
names(dlist) <- regions
    ...
    for(j in 0:7){
      ...
      if(death_type == "totdeath"){
        mod1 <- glm(subdata[, death_type] ~ as.factor(subdata[, paste0("prcp95th_lag", j)]) 
                    + ns(time, 4*6) + ns(subdata[, paste0("templag", j)], 5) + dow + PM25,
...
holder$Analysis <- "PM2.5"
#write.csv(holder, "/Users/kevin/Documents/ChenLabResearch/HurricaneProject/Coding/PostJournalComments/PM25.csv")
```
